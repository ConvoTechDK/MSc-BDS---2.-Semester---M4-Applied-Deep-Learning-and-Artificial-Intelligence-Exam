"""
Assignment 3 - Step 04: Fine-Tune PatentSBERTa v2 (with MAS Gold Labels).

Same architecture as Assignment 2 Step 04, but uses gold labels from the
CrewAI multi-agent system (is_green_human_a3 from mas_labels_100.csv).

Run on HPC: sbatch slurm_finetune_a3.sh
Run locally: python 04_finetune_v2.py
Output: ./results_a3/ (model checkpoint), pushed to Peter512/patentsbert-green-a3
"""
...
MAS_CSV      = os.path.join(os.path.dirname(__file__), 'mas_labels_100.csv')
OUTPUT_DIR   = os.path.join(os.path.dirname(__file__), 'results_a3')
HF_REPO      = "Peter512/patentsbert-green-a3"

print("Loading datasets...")
df_main = pd.read_parquet(PARQUET_PATH)
df_mas  = pd.read_csv(MAS_CSV)

# Merge MAS gold labels: gold overrides silver for the 100 claims
df_main = df_main.copy()
df_main['label'] = df_main['is_green_silver']

gold_lookup = dict(zip(df_mas['doc_id'].astype(str), df_mas['is_green_human_a3'].astype(int)))
...
# Gold_100 with MAS human labels
gold_df = df_mas[['doc_id', 'text', 'is_green_human_a3']].rename(
    columns={'is_green_human_a3': 'label'}
).copy()
...
print("\nEvaluation on gold_100 (MAS labels):")
...
print("\nAssignment 3 fine-tuning complete.")
